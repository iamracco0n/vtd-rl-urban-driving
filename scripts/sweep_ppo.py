...
`--out-root` 에 `--name` 만 바꿔 설정 여러 개를 연달아 돌려도 서로 안 덮어쓴다. 이름이
고정이면 설정을 바꿔 가며 연달아 돌릴 때마다 이전 집계를 조용히 덮어써 마지막 설정만
남는다(에러도 경고도 없다) — 시드별 `summary.json` 은 이름과 무관하게 시드 폴더마다 따로
남으므로, 그렇게 사라진 집계도 `--resume` 으로 학습을 다시 안 돌리고 재조립할 수 있다.
"""
