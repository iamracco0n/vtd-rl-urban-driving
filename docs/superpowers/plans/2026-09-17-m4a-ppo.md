...
- 주의: 워커마다 지도(`rs.load_map()`)를 한 번 읽는다(0.34 초, 프로세스당 한 번). 워커 수는 기본 `min(12, cpu-4)`
...
env -u PYTHONPATH .venv/bin/python scripts/train_ppo.py \
  --out runs/$(hostname)/$(date +%F)-ppo --init runs/lab-main/2026-09-17-dagger-fix2/policy-r4.pt \
  --dagger-data runs/lab-main/2026-09-17-dagger-fix2/data \
  --envs 12 --steps 3000000 --seed 0 | tail -c 800
```
...
- **두 머신 운용**(OMEN 32코어 병렬 수집·학습, `rsync` 로 결과 모으기, `nice` 규칙) — 스펙 §6.5.
