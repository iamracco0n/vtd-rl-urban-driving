...
- Produces:
  - `scripts/train_ppo.py` — 인자 `--out --init runs/…/policy-r4.pt --curricula curricula/stage1.json curricula/stage2.json(기본값, 여러 개) --envs 12 --steps 3000000 --rollout 256 --warmup-updates 5 --eval-every 250000 --eval-seeds 3 --dagger-data runs/…/data --seed 0 --device auto --smoke`
    ...
    - 그 뒤 PPO. `--eval-every` 스텝마다 단계 ①② 평가 → `log.jsonl` 한 줄, 체크포인트 `policy-<스텝>.pt`, 최고 성적 `policy-best.pt`
...
def test_연습_모드가_한_바퀴를_끝낸다(tmp_path):
    ...
    assert os.path.exists(tmp_path / "run" / "policy-best.pt")
...
**Interfaces:**
- Consumes: `train_ppo.py` 가 남긴 `log.jsonl`·`policy-best.pt`, `evaluate_policy`, `evaluate_teacher`, `DrivePolicy.load`
- Produces:
...
2. `scripts/report_m4a.py` 를 쓴다. 학습 곡선 표는 `log.jsonl` 에서, 비교 표와 항목별 위반 표는 `evaluate_policy(DrivePolicy.load(<run>/policy-best.pt), boards, seeds)` 와 `evaluate_policy(DrivePolicy.load(<m3>), …)`, `evaluate_teacher(boards)` 에서 만든다. 목표 네 줄은 위 "목표(성공 기준)" 표의 숫자를 그대로 판정해 `달성`/`미달 (실측값)` 으로 적는다.
